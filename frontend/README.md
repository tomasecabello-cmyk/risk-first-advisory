...
- **Demo de perfil inversor (advisor-friendly)** — entrada principal nueva: un solo card al frente con formulario en español (datos básicos + preguntas abiertas + preferencias), checklist de 8 pasos auto-coloreado y botón **"▶ Ejecutar demo guiada"** que encadena `preparar caso → KYC → análisis IA → aprobación → propuesta → selección → reporte → auditoría` en una sola corrida usando internamente `firm_demo_local` + `advisor_demo_local` + `client_demo_local` del seed. Si OpenAI no está configurado, el paso "Analizar perfil" se reporta como saltado con mensaje amigable y la demo continúa con perfil sugerido razonable. El profesor/asesor nunca ve firms/advisors/clients/case_id ni tokens. Implementado en `frontend/js/investor-demo.js` (prefijo `idemo*`). **El Dashboard técnico y el Workbench paso a paso siguen disponibles más abajo, etiquetados como "Modo avanzado".**
- **Composición de carteras visible** (Fase 3.6): cada variante candidata (DEFENSIVE / BALANCED / GROWTH) muestra una **tabla de holdings** con instrumento + tipo + moneda + peso (barra visual) + motivo. La sección "Propuesta de cartera" del Workbench despliega una tabla comparativa (E[ret], volatilidad, requiere override, # instrumentos) seguida de una **card por variante** con su composición detallada. La "Selección" muestra la composición de la variante elegida. El "Filtro de universo" muestra los primeros 10 instrumentos elegibles con su metadata + chips agrupados con los principales motivos de exclusión (top 5 excluidos colapsados). El **reporte Markdown** incluye una sección "Composición de la cartera seleccionada" (tabla `Instrumento · Tipo · Moneda · Peso · Motivo`) y una "Comparación de variantes generadas" para que el asesor / profesor entienda el trade-off sin abrir Swagger ni leer JSON.
- **UI principal localizada al español** (hero, story-strip, recorrido recomendado, guion de presentación, Workbench, Dashboard, botones, mensajes de carga, labels del resumen). Pensada para mostrarla a un profesor / asesor en Argentina sin necesidad de traducir oralmente. Los **contratos técnicos siguen en inglés** porque son del backend: `case_id`, `firm_id`, `kyc_submission_id`, endpoints `/cases/{case_id}/...`, role names (`admin`/`advisor`/`compliance`/`viewer`), reason_codes, status del FSM (`DRAFT`/`IN_PROGRESS`/`PORTFOLIO_SELECTED`/`CLOSED`), variantes de cartera (`DEFENSIVE`/`BALANCED`/`GROWTH`), JSON keys y nombres de funciones. Los labels del resumen muestran etiqueta en español al lado del nombre técnico (e.g. "ID del caso", "Perfil aprobado", "Cadena de auditoría intacta").
- **Hero + sticky nav + story-strip de 4 cards** ("Qué hace la IA · Qué hace el asesor · Qué controla el sistema · Qué queda auditado") que explica el modelo mental en 10 segundos a un visitante no técnico. CTA "Abrir case_demo_local en el Workbench" salta directo al flujo principal.
- **Recorrido recomendado + Guion de presentación** justo arriba del Dashboard: un checklist de 9 clicks (advisor-friendly) y un guion plegable de 6 frases con el talk-track para presentar el producto a un profesor / asesor no técnico en ~90 segundos. Ver `docs/DEMO_SCRIPT.md` → "Guion advisor-facing — 5 minutos" para la versión expandida.
