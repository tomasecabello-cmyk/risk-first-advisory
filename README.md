...
- **Próximo: Fase 3 — plug-and-play local + Case Workbench frontend** (UI que consuma `/cases/{id}/summary`, seed demo data, bootstrap script). **Primera iteración ya disponible:** el frontend estático incluye una card "Case Dashboard — Phase 2" para listar/crear firms/advisors/clients/cases y cargar el summary del case seleccionado. Las acciones del workflow (KYC, AI analysis, approval, preferences, filter, proposal, override, selection, report) siguen ejercitándose vía `python scripts/run_case_workflow_smoke_check.py` o Swagger `/docs` hasta que el Case Workbench completo aterrice.
